#!/usr/bin/env python3
"""Build a static Saty-style ATR probability ladder using measured TSLA probabilities (Massive)."""
import html
import json
...
from backtest_atr_cascade import LADDER, PDC_IDX, REPORT_LABELS
from massive_pipeline.atr import attach_saty_levels
from massive_pipeline.bars import aggregate_1m_to_3m
from massive_pipeline.splits import apply_split_adjustment, normalize_splits

BASE = Path(__file__).resolve().parent
DATA = json.loads((BASE / 'site/data/atr-cascade-tsla.json').read_text())
OUT = BASE / 'site/atr-levels-probabilities-tsla.html'
ASSET_DIR = BASE / 'site/assets'
ASSET_DIR.mkdir(exist_ok=True)
CACHE = BASE / 'data/massive/cache/TSLA'
START = '2010-06-29'
END = '2026-05-01'

# This Saty-style overall probability chart intentionally uses range-inclusion
# methodology: if the RTH session opens beyond a level, that level counts as hit
# for the day's overall ladder probabilities. The interactive cascade dashboard
# remains sequence-specific and excludes levels already passed at the RTH open.
...
def load_tsla_cached():
    minute = pd.read_csv(CACHE / f'minute_{START}_{END}.csv.gz', parse_dates=['timestamp'])
    daily = pd.read_csv(CACHE / f'daily_{START}_{END}.csv.gz', parse_dates=['timestamp'])
    splits = normalize_splits(pd.read_json(CACHE / f'splits_{START}_{END}.csv.json'), 'TSLA')
    for df in (minute, daily):
        df['date'] = pd.to_datetime(df['timestamp']).dt.date
    minute_adj = apply_split_adjustment(minute, splits, 'date')
    daily_adj = apply_split_adjustment(daily, splits, 'date')
    return attach_saty_levels(aggregate_1m_to_3m(minute_adj), daily_adj)

def build_range_inclusion_paths():
    df = load_tsla_cached()
    built_paths = []
    for _, group in df.groupby('date', sort=True):
        ...
        if seq:
            built_paths.append([i for _, i in sorted(seq)])
    return built_paths, int(df['date'].nunique())

paths, N_DAYS = build_range_inclusion_paths()
...
def arrow(x, start_val, end_val, label, color, side='right', small='', label_x=None, label_y=None, label_anchor=None):
    ...
    <g class="prob-arrow">
      ...
      <text x="{text_x}" y="{text_y:.1f}" text-anchor="{anchor}" class="prob-label">{html.escape(label)}</text>
    </g>'''
...
parts.append(arrow(592,0.382,0.618,f"{fmt_pct(call_gg)} (Golden Gate)",'#75c84a','right',f'+0.382 to +0.618: {fmt_frac(call_gg_num, call_gg_den)} = {call_gg:.1f}%'))
parts.append(arrow(592,-0.382,-0.618,f"{fmt_pct(put_gg)} (Golden Gate)",'#c83d36','right',f'-0.382 to -0.618: {fmt_frac(put_gg_num, put_gg_den)} = {put_gg:.1f}%'))
...
for target, x, txt_y_offset, color in [('+1.00', 782, 0, '#75c84a'), ('-1.00', 782, 0, '#c83d36'), ('+2.00', 930, 0, '#75c84a'), ('-2.00', 930, 0, '#c83d36')]:
    ...
    label_side = 'right'
    parts.append(arrow(x,0,end_val,label,color,label_side,f'PDC to {target}: {fmt_frac(num, den)} = {pct:.1f}%', **major_label_overrides.get(target, {})))
...
legend = f'''
<g class="legend">
  <rect x="1010" y="248" width="244" height="232" rx="5" fill="#0d1316" fill-opacity=".94" stroke="#344047" stroke-width=".85"/>
  <text x="1018" y="266" class="legend-title">TSLA ATR Levels Probabilities</text>
  <text x="1018" y="284" class="legend-line">Overall same-day range chart</text>
  <text x="1018" y="310" class="legend-line">Dataset: TSLA 1m → 3m RTH</text>
  <text x="1018" y="328" class="legend-line">Massive · split-adjusted locally</text>
  <text x="1018" y="346" class="legend-line">2010-07-20 to 2026-05-01</text>
  <text x="1018" y="364" class="legend-line">N = {N_DAYS:,} trading days</text>
  <text x="1018" y="392" class="legend-line">Methodology:</text>
  <text x="1018" y="410" class="legend-line">Counts any level inside RTH range.</text>
  <text x="1018" y="428" class="legend-line">If TSLA opens beyond a level,</text>
  <text x="1018" y="446" class="legend-line">that level counts as hit.</text>
  <text x="1018" y="464" class="legend-line">Arrows = P(target hit | start hit).</text>
</g>'''

footer = '''
<text x="940" y="646" class="footer">Overall chart: gap-open levels are included if inside the day’s RTH range</text>
<text x="940" y="664" class="footer">For clean sequence studies, use the interactive cascade dashboard</text>
...
svg = f'''<svg viewBox="0 0 {W} {H}" role="img" aria-label="TSLA ATR Levels Probabilities static chart">
  <defs>
...
<head>
  ...
  <title>TSLA ATR Levels Probabilities</title>
  <style>
