...
def main():
    ...
    con.execute("""CREATE TABLE IF NOT EXISTS deepitm_legs(
        ticker TEXT, entry_s INTEGER, contract TEXT)""")
    con.execute("DELETE FROM deepitm_legs")
    con.commit()
    ...
    for r in tr.itertuples():
        ...
        cands = [x for x in expirations(r.ticker) if 7 <= (x - ed).days <= 16]
        if not cands:
            skipped += 1
            continue
        exp = min(cands, key=lambda x: (x - ed).days)
        sl = strike_list(r.ticker, exp.isoformat())
        if not sl:
            skipped += 1
            continue
        tgt = r.spot - ITM_DATR * r.datr14_prior
        k = min(sl, key=lambda s: abs(s - tgt))
        cid = f'{r.ticker}|{exp.isoformat()}|{k:.3f}|C'
        con.execute("INSERT INTO deepitm_legs VALUES (?,?,?)",
                    (r.ticker, int(r.entry_s), cid))
